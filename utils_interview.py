...
try:
    ...
    file_handler_main = logging.FileHandler('run_pipeline.log', encoding='utf-8')
...
def get_response_gpt(sys_prompt, inputs, model, retry_count=0, nth_generation=0):
	...
	try:
		...
		# [핵심 수정 부문] 탭(Tab) 들여쓰기로 통일
		if retry_count < 5:
			wait_time = (retry_count + 1) * 10
			logger.warning(f"[OPEN_AI] 에러 발생: {e}. {wait_time}초 후 {retry_count+1}번째 재시도...")
			time.sleep(wait_time)
			return get_response_gpt(sys_prompt, inputs, model, retry_count + 1, nth_generation) 

		print(f'❌ {retry_count}번의 시도 후에도 응답을 받지 못했습니다.')
...
#     # [핵심 수정 부문] 500 에러 및 타임아웃 등 모든 예외에 대한 재시도 강화
#     except Exception as e:
#         logger.exception(e)
        
#         # 재시도 횟수를 5회로 늘리고, 대기 시간도 점진적으로 늘립니다 (Exponential Backoff)
#         if retry_count < 5:
#             wait_time = (retry_count + 1) * 10  # 10초, 20초, 30초... 순으로 대기
#             logger.warning(f"[OPEN_AI] 에러 발생: {e}. {wait_time}초 후 {retry_count+1}번째 재시도...")
#             time.sleep(wait_time)
#             # 다음 시도 시 nth_generation을 높여서 시도하거나 그대로 진행
#             return get_response_gpt(sys_prompt, inputs, model, retry_count + 1, nth_generation) 
