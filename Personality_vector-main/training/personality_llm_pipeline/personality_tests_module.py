...
parser.add_argument('--questionnaire_name', type=str, default='BFI',
                    choices=scale_list,
...
parser.add_argument('--llama_model_path', type=str, default="",
                    help='Path to the fine-tuned model (LLaMA or Qwen)')
...
with open(MODULE_ROOT / 'config.json', 'r') as f:
    config = json.load(f)
...
def personality_assessment(character, agent_type, agent_llm, questionnaire_name,
                           eval_method, evaluator_llm='gpt-3.5-turbo', repeat_times=1):

    # (characters.py 제거에 따라, 별도 캐릭터 로드 없이 진행)
    ...
    final_save_path = os.path.join(final_folder_path, f'{character}_{timestamp}.json')
    ...
    if rerun or not os.path.exists(final_save_path):
        ...
        if eval_method == 'direct':
            # 설문 전체를 한번에 물어보는 direct_ask (원본 유지)
            ...
            if repeat_times < 1:
                subsample_questionnaire_folder_path = os.path.join(
                    PACKAGE_ROOT, 'data', 'subsample_questionnaire', f'ratio={repeat_times}'
                )
            ...
            if agent_llm != 'cAI':
                interview_folder_path = os.path.join(
                    PACKAGE_ROOT, 'results', 'interview',
            ...
            for nth_test in range(max(repeat_times, 1)):
                ...
                # ----- 인터뷰 결과에 대한 LLM 평가(점수 산출) -----
                # (alignment와 무관, 단순 점수 산출)
                assessment_folder_path = os.path.join(
                    PACKAGE_ROOT, 'results', 'assessment',
        ...
        if 'assess' not in eval_method:
            # 원본에서는 choose/convert 시 item_consistency 계산
            # 이 부분은 필요 시 남겨둠
        ...
        # alignment 계산 제거. (원본에는 16Personalities/BFI 코드가 있었지만, 라벨 비교 제거)
        # 아래는 순수한 점수만 남김
        ...
        for analysis, res_v in assessment_results['analysis'].items():
            logger.info(f'{analysis}: {res_v:.5f}' if isinstance(res_v, float) else f'{analysis}: {res_v}')
