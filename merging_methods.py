...
class MergingMethod:
    ...
    def task_arithmetic(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list,
                    scaling_coefficient: float = 1.0, individual_scaling: bool = False, scaling_coefficients: list = None):
        ...
        with torch.no_grad():
            if individual_scaling:
                # 개별 scaling coefficient가 제공되었는지 검증
                assert scaling_coefficients is not None, "scaling_coefficients must be provided when individual_scaling is True"
                assert len(scaling_coefficients) == len(models_to_merge), "The number of scaling coefficients must match the number of models."
                merged_task_vector = None
                # 각 모델의 TaskVector에 개별 scaling을 곱해서 합산
                for i, tv in enumerate(models_to_merge_task_vectors):
                    if merged_task_vector is None:
                        merged_task_vector = scaling_coefficients[i] * tv
                    else:
                        merged_task_vector = merged_task_vector + (scaling_coefficients[i] * tv)
                # 이미 개별 scaling을 적용했으므로 추가 global scaling은 1.0으로 설정
                final_scaling = 1.0
            else:
                # 기존 방식: 모든 TaskVector를 단순 합산
                if len(models_to_merge_task_vectors) == 1:
                ...
                final_scaling = scaling_coefficient

            # 2) (base + merged_task_vector * scale)
            merged_params = merged_task_vector.combine_with_pretrained_model(
                pretrained_model=merged_model,
                scaling_coefficient=final_scaling
            )
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list,
                     ...
                     param_value_mask_rate: float = 0.8, weight_format: str = "delta_weight",
                     weight_mask_rates: list = None, use_weight_rescale: bool = True, mask_strategy: str = "random",
                     mask_apply_method: str = "average_merging", models_use_deepcopy: bool = False,
                     individual_scaling: bool = False, scaling_coefficients: list = None):
        ...
        if self.merging_method_name == "average_merging":
            ...
            with torch.no_grad():
                # 순차적 누적을 위한 aggregator 초기화 (base 모델과 동일한 구조)
                aggregated_delta = {}
                for param_name, param_value in merged_model.named_parameters():
                    aggregated_delta[param_name] = torch.zeros_like(param_value, dtype=param_value.dtype, device=param_value.device)
                
                # 각 fine-tuned 모델에 대해 순차적으로 DaRE 연산을 수행하고 delta를 누적
                for ft_model, weight_mask_rate in zip(models_to_merge, weight_mask_rates):
                    # 현재 fine-tuned 모델에 대해 마스킹된 파라미터(즉, base + masked delta)를 계산
                    ...
                    # 각 파라미터에 대해 (masked 값 - base 값)을 delta로 계산하여 aggregator에 누적
                    for param_name, base_param in merged_model.named_parameters():
                        delta = masked_param_dict[param_name] - base_param
                        aggregated_delta[param_name] += delta  # bf16 정밀도 유지
                    # 현재 모델 처리 후 임시 변수 삭제 및 메모리 회수
                    del masked_param_dict
                    torch.cuda.empty_cache()
                
                # 누적된 delta에 scaling_coefficient를 곱한 후 base 모델에 반영 (task arithmetic)
                for param_name, base_param in merged_model.named_parameters():
                    base_param.add_(aggregated_delta[param_name] * scaling_coefficient)
                # 최종 병합된 모델을 merged_params로 지정
                merged_params = merged_model.state_dict()
                
                # 사용한 aggregator 메모리 해제
